		...
		<Autocomplete options={books} getOptionLabel={(option) => option.title} disableClearable
			...
			renderOption={(properties, option) => (<Stack component="li" {...properties}>
				...
				<Stack direction='row' useFlexGap spacing={2} sx={{ width: '100%' }}>
					<Typography component='div' variant="caption">{option.yearPublished}</Typography>
					<Typography component='div' variant="caption" marginRight='auto' >{formatAuthors(option.authors)}</Typography>
					<Typography component='div' variant="caption">ed.&nbsp;{option.edition}</Typography>
					<Typography component='div' variant="caption">vol.&nbsp;{option.volume}</Typography>
				</Stack>
			</Stack>)}
			renderInput={(params) => (<TextField {...params} label="Book" required={props.required ?? false} inputRef={ref} inputProps={{ ...params.inputProps, 'data-id': bookId }} />)} />
	)
}
